...
Table: (\#tab:simple-table) Data layers used for aquaculture sub-goal
...
$MOMB \ index  = 1 - (\frac{MTB_{farms \ scored \ 3 \ and \ 3}}{MTB_{municipality \ total}})$
...
*Total aquaculture production in northern Norway*
...
In addition to stock sustainability scores, we calculated the annual catch of each species in each municipality and divided it by municipality’s sea area (in $km^2$) to smooth the differences in total catch between the municipalities. There were also considerable interannual differences in total catches (Fig ##), and to smooth them we applied a 3-year rolling mean to catches of each stock using package zoo [@zeileis2005zoo]. Then, we compared catch in each municipality per year with the catch per area of the maximal observed catch per area of the same stock in the whole study region. However, catches between municipalities differed by several powers of magnitude (from less than 10 tons to about 10000 tons), therefore we used a 75th percentile of the total catch distribution per species as a reference maximal catch, and the municipalities that caught more than 75th quantile of the maximal catch, were assigned proportion 1.
